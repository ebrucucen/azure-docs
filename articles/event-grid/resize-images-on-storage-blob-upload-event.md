...
# [Node.js](#tab/nodejs)
The sample Node.js resize function is available on [GitHub](https://github.com/Azure-Samples/storage-blob-resize-function-node). Deploy this Functions code project to the function app by using the [az functionapp deployment source config](/cli/azure/functionapp/deployment/source?view=azure-cli-latest#az-functionapp-deployment-source-config) command.
...
4. Click **Create** to add the event subscription. This creates an event subscription that triggers  `Thumbnail` when a blob is added to the *images* container. The function resizes the images and adds them to the *thumbnails* container.
